...
import app.repeatless.BuildConfig
...
object GeminiAudioClassifier {
    ...
    suspend fun classifySituationText(
        ...
        try {
            ...
            if (!response.isSuccessful || responseString.isBlank()) {
                // Log only metadata in release: response bodies echo the transcript-derived
                // matching context, i.e. content derived from speech in the user's home.
                if (BuildConfig.DEBUG) {
                    Log.e(TAG, "API call failed: ${response.code} $responseString")
                } else {
                    Log.e(TAG, "API call failed: ${response.code} (${responseString.length} bytes)")
                }
                return@withContext fallbackTextMatch(situationText, availablePads)
            }
            ...
            // The request URL carries the API key, and IOException messages can echo it,
            // so only log the throwable itself on debug builds.
            if (BuildConfig.DEBUG) {
                Log.e(TAG, "Error in Gemini text classification", e)
            } else {
                Log.e(TAG, "Gemini text classification failed: ${e::class.java.simpleName}")
            }
            return@withContext fallbackTextMatch(situationText, availablePads)
        }
    }
    ...
    private fun parseGeminiJsonResponse(
        ...
        try {
            ...
            // Never log the raw payload in release — see note above.
            if (BuildConfig.DEBUG) {
                Log.e(TAG, "Error parsing Gemini response: $rawJsonResponse", e)
            } else {
                Log.e(TAG, "Error parsing Gemini response (${rawJsonResponse.length} bytes)", e)
            }
